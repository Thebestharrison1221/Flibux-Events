...
/* Function to fetch avatar URL from Lanyard API or use default */
const getAvatarUrl = async (discordId) => {
    const lanyardApiUrl = `https://api.lanyard.rest/v1/users/${discordId}`;
    try {
        let response = await fetch(lanyardApiUrl);
        let data = await response.json();

        // If user has a custom avatar, return the URL
        if (data && data.data && data.data.discord_user && data.data.discord_user.avatar) {
            const avatarHash = data.data.discord_user.avatar;
            return `https://cdn.discordapp.com/avatars/${discordId}/${avatarHash}.png`;
        } else {
            // Use a default avatar based on user ID
